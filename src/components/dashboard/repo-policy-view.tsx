...
export function RepoPolicyView({ repos }: { repos: DashboardRepo[] }) {
	...
	const [serverState, setServerState] = useState<PolicyFormState>(DEFAULTS);
	const [formState, setFormState] = useState<PolicyFormState>(DEFAULTS);
	const [meta, setMeta] = useState<{
		updatedAt: null | number;
		updatedByLogin: null | string;
		hasDbRow: boolean;
	}>({ hasDbRow: false, updatedAt: null, updatedByLogin: null });
	...
	useEffect(() => {
		...
		const load = async () => {
			try {
				...
				if (!response.ok) {
					toast.error("Couldn't load policy for this repo.");
					return;
				}
				...
				const next = toFormState(data.policy.dbPolicy);
				setServerState(next);
				setFormState(next);
				setMeta({
					hasDbRow: Object.keys(data.policy.dbPolicy).length > 0,
					updatedAt: data.policy.updatedAt,
					updatedByLogin: data.policy.updatedByLogin,
				});
			} finally {
				if (!cancelled) {
					setLoading(false);
	...
	const clear = async () => {
		...
		try {
			...
			setServerState(DEFAULTS);
			setFormState(DEFAULTS);
			setMeta({ hasDbRow: false, updatedAt: null, updatedByLogin: null });
							...
							<div className="flex items-center justify-between">
								<div>
									...
									<div className="mt-0.5 text-[12px] text-muted-foreground">
										...
										"review-needed". Clamped to {MIN_CONFIDENCE}–
										{MAX_CONFIDENCE}.
									</div>
								</div>
